...
for MACHINE in ${MACHINES[@]}; do
	...
	if docker-machine ls | grep $MACHINE | grep -q "Stopped"; then
 	...
 	# Split machine name to get last part
	IFS='.' read -ra MACHINE_SPLIT <<< "$MACHINE"
	MACHINE_WITHOUT_PREFIX=${MACHINE_SPLIT[2]}
	
	MACHINES_IPS+=([${MACHINE_WITHOUT_PREFIX}]=${MACHINE_IP})
done 
...
JERRY_IP=${MACHINES_IPS[jerry]}
TOM_IP=${MACHINES_IPS[tom]}
...
"
eval $(docker-machine env dataclay.federation-demo.citta)
docker run --network=dataclay_default federationdemo/citta src/citta.py register_city

printMsg "2" " 
2. Citta creates and stores TramSystem object named tram-system in its dataClay instance 
"
eval $(docker-machine env dataclay.federation-demo.citta)
docker run --network=dataclay_default federationdemo/citta src/citta.py register_tram_system
...
"
eval $(docker-machine env dataclay.federation-demo.tram)
docker run --network=dataclay_default federationdemo/tram src/tram.py register_tram $TRAM_NAME $CITTA_IP $DATACLAY_PORT
...
when_federated method defined in FermataInfo class (see data model)
 "
eval $(docker-machine env dataclay.federation-demo.fermata)
docker run --network=dataclay_default federationdemo/fermata src/fermata.py register_fermata $FERMATA_NAME $CITTA_IP $DATACLAY_PORT

printMsg "9" " 
9. Citta prints all fermatas and trams available
"
eval $(docker-machine env dataclay.federation-demo.citta)
docker run --network=dataclay_default federationdemo/citta src/citta.py print_city_info
docker run --network=dataclay_default federationdemo/citta src/citta.py print_tram_system_info
...
" 
eval $(docker-machine env dataclay.federation-demo.camera)
docker run --network=dataclay_default federationdemo/camera src/camera.py register_camera $CAMERA_NAME $FERMATA_NAME $FERMATA_IP $DATACLAY_PORT
...
the when_federated method defined in SemaforoInfo class (see data model) 
 " 
eval $(docker-machine env dataclay.federation-demo.semaforo)
docker run --network=dataclay_default federationdemo/semaforo src/semaforo.py register_semaforo $SEMAFORO_NAME $FERMATA_NAME $FERMATA_IP $DATACLAY_PORT
...
"
eval $(docker-machine env dataclay.federation-demo.tram)
docker run --network=dataclay_default federationdemo/tram src/tram.py approach_to_fermata $TRAM_NAME $FERMATA_NAME $FERMATA_IP $DATACLAY_PORT
eval $(docker-machine env dataclay.federation-demo.fermata)
docker run --network=dataclay_default federationdemo/fermata src/fermata.py print_fermata $FERMATA_NAME 
...
"
eval $(docker-machine env dataclay.federation-demo.camera)
docker run --network=dataclay_default federationdemo/camera src/camera.py set_ambulances $CAMERA_NAME 1
...
with the Semaforo
 "
eval $(docker-machine env dataclay.federation-demo.fermata)
docker run --network=dataclay_default federationdemo/fermata src/fermata.py priorize_ambulances $FERMATA_NAME 

printMsg "22" "
22. Semaforo checks the color of the light and set it to red. 
" 
eval $(docker-machine env dataclay.federation-demo.semaforo)
docker run --network=dataclay_default federationdemo/semaforo src/semaforo.py print_color $SEMAFORO_NAME
...
"
eval $(docker-machine env dataclay.federation-demo.camera)
docker run --network=dataclay_default federationdemo/camera src/camera.py set_ambulances $CAMERA_NAME 0
...
"
eval $(docker-machine env dataclay.federation-demo.tram)
docker run --network=dataclay_default federationdemo/tram src/tram.py leave_fermata $TRAM_NAME $FERMATA_NAME $FERMATA_IP $DATACLAY_PORT
