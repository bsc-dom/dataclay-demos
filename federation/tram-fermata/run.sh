...
docker run --rm --network dataclaynetwork dataclaydemo/tram \
	tram/src/tram.py register_tram $TRAM_NAME
...
docker run --rm --network dataclaynetwork dataclaydemo/fermata \
	fermata/src/fermata.py register_fermata $FERMATA_NAME
...
docker run --rm --network dataclaynetwork dataclaydemo/tram \
	tram/src/tram.py approach_to_fermata $TRAM_NAME $FERMATA_NAME $FERMATA_IP $FERMATA_PORT
...
docker run --rm --network dataclaynetwork dataclaydemo/fermata \
	fermata/src/fermata.py print_fermata $FERMATA_NAME 
...
docker run --rm --network dataclaynetwork dataclaydemo/fermata \
	fermata/src/fermata.py set_ambulances $FERMATA_NAME 1
...
docker run --rm --network dataclaynetwork dataclaydemo/tram \
	tram/src/tram.py leave_fermata $TRAM_NAME $FERMATA_NAME $FERMATA_IP $FERMATA_PORT
