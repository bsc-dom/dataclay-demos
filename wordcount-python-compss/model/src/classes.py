...
class TextCollection(DataClayObject):
    ...
    @ClassField texts list<DemoNS.classes.Text>
    """
    @dclayMethod()
    def __init__(self):
        self.texts = list()
    ...
    def add_text_from_file(self, text_file):
        ...
        with open(text_file, "r") as f:
                for line in f.readlines():
                    text.add_words(line.strip().lower().replace(".","'").split())
    ...
    def add_text(self, text):
        ...
        text_obj = Text()
        text_obj.add_words(text.strip().lower().replace(".","'").split())
...
class TextStats(DataClayObject):
    ...
    @dclayMethod(target="DemoNS.classes.TextStats")
    ...
    def get_summary(self, num_words):
        sorted_values = sorted((count, word) for word, count in self.current_word_count.items())
...
class Text(DataClayObject):
    ...
    @dclayMethod(return_="DemoNS.classes.TextStats")
    def word_count(self):
        partialResult = dict()
